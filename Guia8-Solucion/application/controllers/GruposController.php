...
class GruposController extends CI_Controller
{
    ...
    public function report_todos_los_grupos()
    {
        ...
        foreach ($grupos as $grupo):
            foreach ($materias as $materia):
                if($grupo->idmateria == $materia->idmateria){
                    foreach ($profesores as $profesor):
                        if($grupo->idprofesor == $profesor->idprofesor){
                            $this->table->add_row($grupo->idgrupo, $grupo->num_grupo, $grupo->anio, $grupo->ciclo, $materia->materia, $profesor->nombre.' '.$profesor->apellido);
                        }
                    endforeach;
                }
            endforeach;
    ...
    public function report_todos_los_estudiantes_grupo($id)
    {
        // Se carga la libreria para generar tablas
        $this->load->library("table");
        //Carga la librería Report que acabamos de crear
        ...
        $pdf = new Report(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        $pdf->titulo = "Listado de Estudiantes del Grupo";
        // Información del documento
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('Oscar Rivera');
        $pdf->SetTitle('Listado de Estudiantes del Grupo');
        $pdf->SetSubject('Reporte generado usando Codeigniter y TCPDF');
        $pdf->SetKeywords('TCPDF, PDF, MySQL, Codeigniter');
        ...
        $this->table->set_heading('Id Grupo', 'Numero de Grupo', 'Id Estudiante', 'Nombre Completo');
    
        // Cargando la data
        $this->load->model('GruposModel');

        // Asignando la data
        $grupo_estudiantes = $this->GruposModel->getEstudiantesByIdGrupo2($id);
    
        // Iterando sobre la data
        foreach ($grupo_estudiantes as $grupo_estudiante):
            $this->table->add_row($grupo_estudiante->idgrupo, $grupo_estudiante->num_grupo, $grupo_estudiante->idestudiante, $grupo_estudiante->nombrecompleto);
        endforeach;
